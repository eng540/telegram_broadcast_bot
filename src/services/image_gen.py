...
class ImageGenerator:
    ...
    def _create_template(self):
        ...
        <head>
            ...
            <style>
                ...
                body {
                    margin: 0; padding: 0; width: 1080px; min-height: 1350px;
                    background-color: #ffffff; font-family: 'Amiri', serif;
                    display: flex; flex-direction: column; align-items: center;
                    justify-content: space-between; color: #2c1e18;
                    box-sizing: border-box; padding-bottom: 50px;
                }
                .main-content {
                    width: 850px; padding-top: 150px; padding-bottom: 100px;
                    flex-grow: 1; display: flex; flex-direction: column;
                    justify-content: center; align-items: center;
                }
                .text-body {
                    font-size: {{ font_size }}px; font-weight: 700; line-height: 1.8;
                    text-align: center; white-space: pre-wrap;
                }
                .footer-container {
                    width: 600px; margin-top: 50px; display: flex;
                    flex-direction: column; align-items: center; opacity: 0.9;
                    flex-shrink: 0;
                }
                .divider { width: 100%; display: flex; align-items: center; justify-content: center; margin-bottom: 25px; }
                .line { height: 2px; background-color: #bcaaa4; flex-grow: 1; border-radius: 2px; }
                .ornament { padding: 0 15px; color: #8d6e63; font-size: 24px; }
                .brand-name { font-size: 38px; font-weight: 700; color: #3e2723; margin-bottom: 10px; }
                .handle-box {
                    display: flex; align-items: center; background-color: #f5f5f5;
                    padding: 8px 25px; border-radius: 50px; border: 1px solid #e0e0e0;
                }
                .handle-text { font-size: 24px; color: #0088cc; font-weight: 600; direction: ltr; margin-left: 10px; }
            </style>
        </head>
        <body>
            <div class="main-content"><div class="text-body">{{ text }}</div></div>
            <div class="footer-container">
                <div class="divider"><div class="line"></div><div class="ornament">✦</div><div class="line"></div></div>
                <div class="brand-name">{{ channel_name }}</div>
                <div class="handle-box"><span class="handle-text">{{ channel_handle }}</span></div>
            </div>
        </body>
        </html>
        """
    ...
    async def render(self, text: str, message_id: int) -> str:
        logger.info(f"🎨 Rendering: {message_id}")
        text_len = len(text)
